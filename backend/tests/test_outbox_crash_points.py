...
async def _enqueue(db: AsyncSession, count: int) -> None:
...
async def test_a_crash_before_the_outcome_costs_one_message_not_the_batch(
    ...
    async with db_factory() as restarted:
        stats = await drain_outbox(
            restarted, {"email": second}, now=datetime.now(UTC) + DEFAULT_LEASE
        )
...
async def test_a_claimed_row_is_never_lost_only_delayed_by_its_lease(db, db_factory) -> None:
    ...
    async with db_factory() as later:
        await drain_outbox(later, {"email": provider}, now=datetime.now(UTC) + DEFAULT_LEASE)
...
async def test_a_claim_holds_the_row_only_for_its_lease(db, db_factory) -> None:
...
async def test_every_attempt_at_one_message_carries_the_same_key(db, db_factory) -> None:
    ...
    for offset in (timedelta(0), DEFAULT_LEASE, DEFAULT_LEASE * 2):
        async with db_factory() as worker:
            with pytest.raises(_Crash):
                await drain_outbox(worker, {"email": provider}, now=datetime.now(UTC) + offset)
