...
if [ "$failed" -ne 0 ]; then
  echo "Keep read_only: true, cap_drop: [ALL] and no-new-privileges:true on every service (docker-compose.yml)." >&2
  exit 1
fi
echo "compose hardening: every service is read-only, drops all capabilities and sets no-new-privileges"
